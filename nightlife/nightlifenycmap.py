    ...
    if len(values) == 19 or len(values)==18:# yellowtaxi_data
        ...
        if(((dropoff_hour>=21 and dropoff_hour<=23) or (dropoff_hour>=0 and dropoff_hour<=2)) and (dateset.weekday() in weekend)):
            if "Manhattan" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Manhattan,%.3f,%.3f,y\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Bronx" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Bronx,%.3f,%.3f,y\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Brooklyn" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Brooklyn,%.3f,%.3f,y\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Queens" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Queens,%.3f,%.3f,y\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Staten Island" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Staten Island,%.3f,%.3f,y\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))


    elif len(values) == 23: #green taxi
        ...
        #        print(dateset.weekday())
        dropoff_hour = int(datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').strftime('%H'))
        if((dropoff_hour>=21 and dropoff_hour<=23) or (dropoff_hour>=0 and dropoff_hour<=2) and (dateset.weekday() in weekend)):
            if "Manhattan" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Manhattan,%.3f,%.3f,g\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Bronx" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Bronx,%.3f,%.3f,g\t%d" % (dropoff_month,float(values[7]),float(values[8]),1))
            if "Brooklyn" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Brooklyn,%.3f,%.3f,g\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Queens" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Queens,%.3f,%.3f,g\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Staten Island" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Staten Island,%.3f,%.3f,g\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
