...
for line in sys.stdin:
    ...
    if len(values) == 19 or len(values)==18 :# yellowtaxi_data
        ...
        point1=Point(float('%.5f'%float(values[6])),float('%.5f'%float(values[5])))
        pickup_neighborhood = findNeighborhood(pickup_location, index, neighborhoods)
        pickup_datetime = values[1]
        pickup_year=((datetime.strptime(pickup_datetime, '%Y-%m-%d %H:%M:%S').strftime('%Y')))
        ...
        dateset=datetime(int(pickup_year),int(pickup_month),int(pickup_date))
        pickup_hour = int(datetime.strptime(pickup_datetime, '%Y-%m-%d %H:%M:%S').strftime('%H'))
        ...
        if (dateset.weekday() in weekend):
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,y,weekend\t%d" % (pickup_hour,1))
            if "Manhattan" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Manhattan,y,weekend\t%d" % (pickup_hour,1))
            if "Bronx" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Bronx,y,weekend\t%d" % (pickup_hour,1))
            if "Brooklyn" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Brooklyn,y,weekend\t%d" % (pickup_hour,1))
            if "Queens" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Queens,y,weekend\t%d" % (pickup_hour,1))
            if "Staten Island" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Staten Island,y,weekend\t%d" % (pickup_hour,1))
        else:
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,y,weekday\t%d" % (pickup_hour,1))
            if "Manhattan" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Manhattan,y,weekday\t%d" % (pickup_hour,1))
            if "Bronx" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Bronx,y,weekday\t%d" % (pickup_hour,1))
            if "Brooklyn" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Brooklyn,y,weekday\t%d" % (pickup_hour,1))
            if "Queens" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Queens,y,weekday\t%d" % (pickup_hour,1))
            if "Staten Island" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Staten Island,y,weekday\t%d" % (pickup_hour,1))


    elif len(values) == 23 or len(values)==22:
        pickup_location = (float(values[5]), float(values[6]))
        point1=Point(float('%.5f'%float(values[6])),float('%.5f'%float(values[5])))
        pickup_neighborhood = findNeighborhood(pickup_location, index, neighborhoods)
        pickup_datetime = values[1]
        ...
        if (dateset.weekday() in weekend):
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,g,weekend\t%d" % (pickup_hour,1))
            if "Manhattan" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Manhattan,g,weekend\t%d" % (pickup_hour,1))
            if "Bronx" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Bronx,g,weekend\t%d" % (pickup_hour,1))
            if "Brooklyn" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Brooklyn,g,weekend\t%d" % (pickup_hour,1))
            if "Queens" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Queens,g,weekend\t%d" % (pickup_hour,1))
            if "Staten Island" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Staten Island,g,weekend\t%d" % (pickup_hour,1))
        else:
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,g,weekday\t%d" % (pickup_hour,1))
            if "Manhattan" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Manhattan,g,weekday\t%d" % (pickup_hour,1))
            if "Bronx" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Bronx,g,weekday\t%d" % (pickup_hour,1))
            if "Brooklyn" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Brooklyn,g,weekday\t%d" % (pickup_hour,1))
            if "Queens" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Queens,g,weekday\t%d" % (pickup_hour,1))
            if "Staten Island" in neighborhoods[pickup_neighborhood][0]:
                print ("%s,Staten Island,g,weekday\t%d" % (pickup_hour,1))
