...
for line in sys.stdin:
    ...
    if values[0]=="VendorID" or values[0]=="vendor_id": continue
    ...
    if len(values) == 19 or len(values)==18:# yellowtaxi_data
        ...
        dropoff_year=((datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').strftime('%Y')))
        dropoff_month = int(datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').strftime('%m'))
        dropoff_date = int(datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').strftime('%d'))
        dateset=datetime(int(dropoff_year),int(dropoff_month),int(dropoff_date))
        dropoff_hour = int(datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').strftime('%H'))
        point1=Point(float('%.5f'%float(values[10])),float('%.5f'%float(values[9])))

        if(((dropoff_hour>=21 and dropoff_hour<=23) or (dropoff_hour>=0 and dropoff_hour<=2)) and (dateset.weekday() in weekend)):
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,%.3f,%.3f,y,weekend\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
                continue
            if "Manhattan" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Manhattan,%.3f,%.3f,y,weekend\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Bronx" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Bronx,%.3f,%.3f,y,weekend\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Brooklyn" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Brooklyn,%.3f,%.3f,y,weekend\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Queens" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Queens,%.3f,%.3f,y,weekend\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Staten Island" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Staten Island,%.3f,%.3f,y,weekend\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))

        elif(((dropoff_hour>=21 and dropoff_hour<=23) or (dropoff_hour>=0 and dropoff_hour<=2)) and (dateset.weekday() not in weekend)):
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,%.3f,%.3f,y,weekday\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
                continue
            if "Manhattan" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Manhattan,%.3f,%.3f,y,weekday\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Bronx" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Bronx,%.3f,%.3f,y,weekday\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Brooklyn" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Brooklyn,%.3f,%.3f,y,weekday\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Queens" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Queens,%.3f,%.3f,y,weekday\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))
            if "Staten Island" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Staten Island,%.3f,%.3f,y,weekday\t%d" % (dropoff_hour,float(values[9]),float(values[10]),1))


    elif len(values) == 23 or len(values)==22: #green taxi
        ...
        dateset=datetime(int(dropoff_year),int(dropoff_month),int(dropoff_date))
        dropoff_hour = int(datetime.strptime(dropoff_datetime, '%Y-%m-%d %H:%M:%S').strftime('%H'))
        point1=Point(float('%.5f'%float(values[8])),float('%.5f'%float(values[7])))
        if(((dropoff_hour>=21 and dropoff_hour<=23) or (dropoff_hour>=0 and dropoff_hour<=2)) and (dateset.weekday() in weekend)):
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,%.3f,%.3f,g,weekend\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
                continue
            if "Manhattan" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Manhattan,%.3f,%.3f,g,weekend\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Bronx" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Bronx,%.3f,%.3f,g,weekend\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Brooklyn" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Brooklyn,%.3f,%.3f,g,weekend\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Queens" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Queens,%.3f,%.3f,g,weekend\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Staten Island" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Staten Island,%.3f,%.3f,g,weekend\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))

        elif(((dropoff_hour>=21 and dropoff_hour<=23) or (dropoff_hour>=0 and dropoff_hour<=2)) and (dateset.weekday() not in weekend)):
            if poly.contains(point1) or poly1.contains(point1):
                print ("%s,Queens,%.3f,%.3f,g,weekday\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
                continue
            if "Manhattan" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Manhattan,%.3f,%.3f,g,weekday\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Bronx" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Bronx,%.3f,%.3f,g,weekday\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Brooklyn" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Brooklyn,%.3f,%.3f,g,weekday\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Queens" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Queens,%.3f,%.3f,g,weekday\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
            if "Staten Island" in neighborhoods[dropoff_neighborhood][0]:
                print ("%s,Staten Island,%.3f,%.3f,g,weekday\t%d" % (dropoff_hour,float(values[7]),float(values[8]),1))
